...
<script type="text/javascript">
	//date Picker to show date of travel for a week starting from the next date of today
	$(function() {
...
<script type="text/javascript">
	...
	function extract() {
		...
		if (urlindex > -1) {
			url1 = url.substring(0, urlindex);
			window.location.replace(url1 + "?sid=" + sid + "&did=" + did
					+ "&des=" + val1);
		} else {
			window.location.replace(url + "?sid=" + sid + "&did=" + did
					+ "&des=" + val1);
		}
	}
	...
	function search() {
		...
		if (src == 0) {
			...
			if (urlindex > -1) {
				url1 = url.substring(0, urlindex);
				window.location.replace(url1 + "?sid=" + src + "&did=" + des
						+ "&date=" + date);
			} else {
				window.location.replace(url + "?sid=" + src + "&did=" + des
						+ "&date=" + date);
			}
		}
	}
			...
			<div align="center"
				...
				<%
					...
					try {
						if (session.isNew()) {
							out.println(
									"<h3><span style='color: red;'>Access Denied Only Registered Users Can Make The Booking</span></h3>");
						} else {
				...
				<form name="form1" action="./Book.jsp">
					...
					<table>
						<tr align="left">
							<td><label style="font-weight: bold;"> Source
									Station </label></td>
							...
							<td>
								...
								%> <select id="select1" name="source" onchange="change();"
								onfocus="back()">
							...
							</select> <!-- JavaScript To Eliminate Similar Station Names Generated From BackEnd -->
								<script type="text/javascript">
									function back() {
										var sel = document
								...
								</script> <!-- End JavaScript To Eliminate Similar Station Names Generated From BackEnd -->
						...
						<tr align="left">
							<td><label style="font-weight: bold;"> Destination
									Station </label></td>

							<td><select id="select2" name="destid" onchange="extract()">
									<option value="0">--Please Select--</option>
									<%
					...
					<table border="1" bordercolor=#333333>
						...
						<tr>
							...
							<td><input type="hidden" name="source1" readonly="readonly"
								value="<%=sid%>" /> <input type="hidden" name="destination1"
								readonly="readonly" value="<%=did%>" /> <input type="text"
								readonly="readonly" value="<%=src%>" /></td>
						...
						<tr align="left">
							...
							<td><input type="text" name="date" readonly="readonly"
								id="datepicker" title="YYYY/MM/DD"
									...
									try {
										...
										while (rs.next()) {
											...
											if (rs.getString(week_travel).equalsIgnoreCase("Y")) {
												...
												String sql5 = "select dep_date from s1 where train_no=" + rs.getString("train_no")
														+ " and dep_date='" + dt + "';";
												...
												if (flag == 0) {
													...
													for (int i = 1; i <= a1; i++) { //Inserting values into ac first class i.e tables with "a1" type coach no
														String sql6 = "insert into a" + i + " (train_no, dep_date) values ("
																+ rs.getString("train_no") + ",'" + dt + "')";
													...
													for (int i = 1; i <= sl; i++) { //Inserting values into Sleeper class i.e tables with "s1" type coach no
														String sql7 = "insert into s" + i + " (train_no, dep_date) values ("
																+ rs.getString("train_no") + ",'" + dt + "')";
													...
													for (int i = 1; i <= a2; i++) { //Inserting values into AC 2 Tier Class i.e tables with "b1" type coach no
														String sql8 = "insert into b" + i + " (train_no, dep_date) values ("
																+ rs.getString("train_no") + ",'" + dt + "')";
													...
													for (int i = 1; i <= a3; i++) { //Inserting values into AC 3 Tier class i.e tables with "c1" type coach no
														String sql9 = "insert into c" + i + " (train_no, dep_date) values ("
																+ rs.getString("train_no") + ",'" + dt + "')";
										...
										while (rs.next()) {
											...
											out.println(
													"<td>  <input type='radio' name='tno' size='5' value=" + rs.getInt("train_no")
															+ " onclick='next()' /> " + rs.getInt("train_no") + " </td> ");
											out.println("<td>  <input type='text' value=" + rs.getString("train_name")
													+ " readonly='readonly' /> </td>");
											out.println("<td>  <input type='text' size='10' value=" + rs.getString("type")
													+ " readonly='readonly' /> </td>");
											out.println("<td>  <input type='text' name='date1' size='10' value="
													+ rs.getDate("dep_date") + " readonly='readonly' /> </td>");
											out.println("<td>  <input type='text' size='15' value=" + rs.getString("s_station_name")
													+ " readonly='readonly' /> </td>");
											out.println("<td>  <input type='text' size='15' value=" + rs.getString("d_station_name")
													+ " readonly='readonly' /> </td>");
											out.println("</tr>");
					%>
					<!-- 			<tr>
