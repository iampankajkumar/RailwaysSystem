...
<script type="text/javascript">
	function call() {
		...
		if (urlindex > -1) {
			...
			window.location.replace(url1 + "?tno=" + tno + "&date1=" + date1
					+ "&clas=" + clas + "&adult=" + adult + "&child=" + child
					+ "&src=" + src + "&des=" + des + "&ch=" + val);
		} else {
			window.location.replace(url + "?tno=" + tno + "&date1=" + date1
					+ "&clas=" + clas + "&adult=" + adult + "&child=" + child
					+ "&src=" + src + "&des=" + des + "&ch=" + val);
		}
	}
...
<body>
			...
			<div align="center">
				...
				<form name="form1" action="./Book3.jsp">
					...
					<table border="1" bordercolor=#333333>
						<tr bgcolor=#333333>
							<th colspan="6" align="left" style="color: white;">Train
								Details :</th>
						</tr>
						...
						<tr>
							...
							<td><input type="text" size="9"
								value="<%=rs.getString("distance")%>" readonly="readonly"
								tabindex="-1" /></td>
					...
					<input name="adult1" type="hidden" value="<%=adult%>" /> <input
						name="child1" type="hidden" value='<%=child%>' /> <input
						name="clas1" type="hidden" value='<%=clas%>' /> <input name="src"
						type="hidden" value='<%=srcid%>' /> <input name="des"
						type="hidden" value='<%=desid%>' />
					...
					<table border="1" bordercolor=#3333333>
						...
						<tr>
							<th>Travel Date:</th>
							<td><input type="text" name="date1" size="9" value="<%=dt%>"
								readonly="readonly" tabindex="-1" /></td>
	...
	<option value="s3"> s3 </option>
					...
					<p>
						<input type="button" value="Continue Reservation"
							onclick="validate()" />
					</p>
